...
var _commonProto = protogen.common;
var _configtxProto = protogen.common;
var _ccProto = protogen.protos;
var _queryProto = protogen.protos;

var config = sdkUtils.getConfig();
// setup the location of the default config shipped with code
var default_config = path.resolve( __dirname, '../config/default_client.json');
config.reorderFileStores(default_config); //make sure this default has precedences
// set default SSL ciphers for gRPC
// not allowed in javascript: // process.env.GRPC_SSL_CIPHER_SUITES = sdkUtils.getConfigSetting('grpc-ssl-cipher-suites');
...
var Client = class extends BaseClient {
	...
	setStateStore(keyValueStore) {
		...
		if (global.isJsLibraryMode) {
			//load key value stora that uses store.js
			methods = sdkUtils.getClassMethods(api.BrowserKeyValueStore);
		} else {
			methods = sdkUtils.getClassMethods(api.KeyValueStore);
		}
	...
	saveUserToStateStore() {
		...
		return new Promise(function(resolve, reject) {
			if (self._userContext && self._userContext._name && self._stateStore) {
				...
				if (global.isJsLibraryMode) {
					self._stateStore.set(self._userContext._name, self._userContext.toString());
					logger.debug('saveUserToStateStore, store.setValue, result = ' + self._userContext.toString());
	...
	loadUserFromStateStore(name) {
		...
		return new Promise(function(resolve, reject) {
			...
			if (global.isJsLibraryMode) {
				logger.debug('loadUserFromStateStore, loading user state from browser store');
				//this read will not return a promise, just simple data
				var readedUsernameData = self._stateStore.get(name);
				var data = null;
				if (readedUsernameData) {
					...
					data = newUser.fromString(readedUsernameData, true);
				} else {
				...
				if(global.isJsLibraryMode){
					let memberStr = self._stateStore.get(name);
					if (memberStr) {
						// The member was found in the key value store, so restore the state.
						const newUser = new User(name);
						if (!self.getCryptoSuite()) {
