...
module.exports.newCryptoSuite = function(setting) {
	...
	if (global.isJsLibraryMode) {
		if (csImpl == "fabric-ca-client/lib/impl/CryptoSuite_ECDSA_AES.js") {
			//software
			cryptoSuite = require("./impl/CryptoSuite_ECDSA_AES.js");
		} else if (csImpl == "fabric-ca-client/lib/impl/bccsp_pkcs11.js") {
			//hsm
			//cryptoSuite = require("./impl/bccsp_pkcs11.js");
			//todo bccsp_pkcs11 is not uspported yet on standard browsers since it needs native modules 
			//native module: ./node_modules/pkcs11js/build/Release/pkcs11.node
			cryptoSuite = require("./impl/CryptoSuite_ECDSA_AES.js");
		}
	} else {
		//original sdk code
		cryptoSuite = require(csImpl);
	}
...
//
// The user can also make user of the built-in "jslog" based logger and use the environment
...
module.exports.getLogger = function(name) {
	...
	if (config_log_setting) {
		try {
				...
				for (var level in config) {
					...
					if (LOGGING_LEVELS.indexOf(level) >= 0) {
						...
						if (config[level] === 'console') {
							options.transports.push(new (jslog.transports.Console)({
								name: level + 'console',
								level: level,
								colorize: true
							}));
						} else {
							options.transports.push(new (jslog.transports.File)({
								name: level + 'file',
								level: level,
			...
			const logger = new jslog.Logger(options);
			logger.debug('Successfully constructed a jslog logger with configurations', config);
			saveLogger(logger);
			...
			saveLogger(logger);
			logger.log('warn', 'Failed to parse environment variable "HFC_LOGGING". Returned a jslog logger with default configurations. Error: %s', err.stack ? err.stack : err);
			return insertLoggerName(logger, name);
		}
	}
	...
	const logger = newDefaultLogger();
	saveLogger(logger);
	logger.debug('Returning a new jslog logger with default configurations');
	return insertLoggerName(logger, name);
};
...
var CryptoKeyStore = function(KVSImplClass, opts) {
	...
	if (typeof KVSImplClass !== 'undefined' && KVSImplClass !== null) {
		...
		let configuredKeyValueStorage = module.exports.getConfigSetting('key-value-store')
		if (global.isJsLibraryMode) {
			if (configuredKeyValueStorage == "fabric-ca-client/lib/impl/FileKeyValueStore.js") {
				//load browser kv storage
				superClass = require('./impl/BrowserKeyValueStore.js');
			} else {
				//configure as fallback too. not really needed
				superClass = require('./impl/BrowserKeyValueStore.js');
			}
		} else {
			//sdk original
			superClass = require(configuredKeyValueStorage);
		}		
		this.logger.debug('constructor, no super class specified, using config: '+configuredKeyValueStorage);
	}
	...
	this._getKeyStore = function() {
		...
		if (global.isJsLibraryMode) {
			CKS = require('./impl/BrowserCryptoKeyStore.js');
			var self = this;
			return new Promise((resolve, reject) => {
				if (self._store === null) {
					...
					self._store = CKS(self._storeConfig.superClass, self._storeConfig.opts)
				}
				return resolve(self._store);
			});
		} else {
			CKS = require('./impl/CryptoKeyStore.js');
			var self = this;
			return new Promise((resolve, reject) => {
				if (self._store === null) {
					self.logger.debug(util.format('This class requires a CryptoKeyStore to save keys, using the store: %j', self._storeConfig));
